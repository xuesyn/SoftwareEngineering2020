...
function initData(vueOptions, context) {
  ...
  if (typeof data === 'function') {
    try {
      ...
      if (Object({"NODE_ENV":"development","VUE_APP_NAME":"GoEasyDemo-Uniapp-LiveChatRoom","VUE_APP_PLATFORM":"mp-weixin","BASE_URL":"/"}).VUE_APP_DEBUG) {
...
/* 4 */
/*!**********************************************************************!*\
  !*** C:/Users/pro/Desktop/GoEasyDemo-Uniapp-LiveChatRoom/pages.json ***!
  \**********************************************************************/
/*! no static exports found */
/***/ (function(module, exports) {
...
/* 25 */
/*!******************************************************************************!*\
  !*** C:/Users/pro/Desktop/GoEasyDemo-Uniapp-LiveChatRoom/lib/chatservice.js ***!
  \******************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
var _goeasyIm = _interopRequireDefault(__webpack_require__(/*! ./goeasy-im-1.0.9.js */ 26));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} /*
                                                                                                                                                                       * @Author: jack.lu
                                                                                                                                                                       * @Date: 2020-4-21 10:10:20
                                                                                                                                                                       * @Last Modified by: jack.lu
                                                                                                                                                                       * @Last Modified time: 2020-4-21 15:01:41
                                                                                                                                                                       */ //用户
function User(id, nickname, avatar, value, score, ifsubmit, ifcheck) {this.id = id;this.nickname = nickname;this.avatar = avatar;
  this.value = value;
  this.score = score;
...
//获取实例
ChatRoomService.prototype.connectGoEasyIM = function () {var _this = this;
  this.im = _goeasyIm.default.getInstance({
  ...
  this.im.on(_goeasyIm.default.EVENT.CONNECTED, function () {
    ...
    //连接成功后，更新在线用户数和用户头像
    _this.initialOnlineUsers(_this.room.id);
  });
  ...
  then(function () {
    console.log('连接成功');
    _this.initialChatHistory(_this.room.id);

    //订阅用户上下线事件
    _this.subscribePresence(_this.room.id);
    //订阅聊天室消息
    _this.subscribeRoomMessage(_this.room.id);
...
//监听新消息
ChatRoomService.prototype.listenerNewMessage = function () {var _this2 = this;

  this.im.on(_goeasyIm.default.EVENT.GROUP_MESSAGE_RECEIVED, function (message) {
    var content = JSON.parse(message.payload.text);
    //console.log(message);
    _this2.addNewMessage(message);
    _this2.whenNewMessage(content);
  });
};
...
ChatRoomService.prototype.addNewMessage = function (message) {
  ...
  if (content.type == this.room.MessageType.PROP && this.room.ifask == 0) {
    ...
    for (var i = 0; i < roomMessage.count; i++) {
      var _newLog = new Log(roomMessage.users[i].nickname);
      this.room.res.series.push(_newLog);
    }
    var newLog = new Log('黄金点');
    ...
    var newMessage = new Message(message.senderId, content.senderNickname, messageContent);
  ...
  if (content.type == this.room.MessageType.CHECK) {
    for (var i = 0; i < roomMessage.count; i++) {
      if (roomMessage.users[i].id == message.senderId) {
        ...
        var _messageContent = "";
        _messageContent = "准备就绪";
        var _newMessage = new Message(message.senderId, content.senderNickname, _messageContent);
        this.room.messages.push(_newMessage);
        break;
      }
    }
  }
  ...
  if (!this.room.running && this.room.ifask) {
    if (roomMessage.count == this.room.countCheck) {
      this.room.running = 1;
      var _messageContent2 = "";
      _messageContent2 = "全员就绪,请输入0-100间数字,努力猜到黄金点吧";
      this.room.currentTurn = 0;
      this.room.ifask = 0;
      this.room.countCheck = 0;
      flag = 1;
      var _newMessage2 = new Message(message.senderId, "", _messageContent2);
      this.room.messages.push(_newMessage2);
    } else {
      var _messageContent3 = "";
      _messageContent3 = "还有" + (roomMessage.count - this.room.countCheck) + "人未确认";
      flag = 1;
      var _newMessage3 = new Message(message.senderId, "黄金点提醒您", _messageContent3);
      this.room.messages.push(_newMessage3);
    }
  }

  if (this.room.ifask == 0 && this.room.running == 0) {
    var _newMessage4 = new Message(message.senderId, content.senderNickname, content.content);
    this.room.messages.push(_newMessage4);
  }
  ...
  if (!isNaN(number) && flag == 0 && this.room.running == 1) {
    ...
    if (ifAllSubmit == 1) {
      ...
      for (var i = 0; i < roomMessage.count; i++) {
        ...
        _messageContent4 = _messageContent4 + roomMessage.users[i].nickname + ": " + roomMessage.users[i].score + " || ";
      }
      //聊天消息
      ...
      var _newMessage5 = new Message(message.senderId, "本轮结果", _messageContent4);
      this.room.messages.push(_newMessage5);
      var newMessage1 = new Message(message.senderId, "Golden Points", "" + average);
      this.room.messages.push(newMessage1);
      console.log("currentGameTurns:");
      console.log(this.room.currentTurn);
      for (var i = 0; i < roomMessage.count; i++) {
        var _newLog3 = new Log(roomMessage.users[i].nickname);
        _newLog3 = this.room.res.series[i];
        _newLog3.data.push(parseFloat(roomMessage.users[i].value));
        this.room.res.series[i] = _newLog3;
      }
      var _newLog2 = new Log("");
      _newLog2 = this.room.res.series[roomMessage.count];
      _newLog2.data.push(average);
      this.room.res.series[roomMessage.count] = _newLog2;
      console.log("Series:");
      console.log(this.room.res.series);
      if (this.room.currentTurn == this.room.totalTurn) {
        ...
        this.room.currentTurn = 0;
        var _newMessage6 = new Message(message.senderId, "黄金点提醒您", "本轮游戏结束");
        this.room.messages.push(_newMessage6);
      }
      //let newMessage = new Message(message.senderId, content.senderNickname, messageContent);
      //this.room.messages.push(newMessage);
    } else {
      var _newMessage7 = new Message(message.senderId, "等待其他玩家输入中", " 还有" + countNum + "名玩家未提交");
      this.room.messages.push(_newMessage7);
    ...
    var _newMessage8 = new Message(message.senderId, "黄金点提醒您", "请输入0-100内的自然数");
    this.room.messages.push(_newMessage8);
  }
};

//监听用户上下线
ChatRoomService.prototype.listenerGroupPresence = function () {var _this3 = this;
  this.im.on(_goeasyIm.default.EVENT.GROUP_PRESENCE, function (event) {
    //更新在线用户数
    _this3.room.onlineUsers.count = event.groupOnlineCount;

    if (event.action == 'join' || event.action == 'online') {
      var userData = JSON.parse(event.userData);
      //添加新用户
      var user = new User(event.userId, userData.nickname, userData.avatar, "", 0, 0, 0);

      //添加在线用户，避免用户重复
      if (!_this3.room.onlineUsers.users.find(function (item) {return item.id == event.userId;})) {
        _this3.room.onlineUsers.users.push(user);
      }

      //添加进入房间的消息
      var message = new Message(event.userId, userData.nickname, " 进入房间", _this3.room.MessageType.CHAT);
      _this3.room.messages.push(message);
    } else {
      var offlineUserIndex = _this3.room.onlineUsers.users.findIndex(function (item) {return item.id == event.userId;});
      if (offlineUserIndex > -1) {
        //将离开的用户从onlineUsers中删掉
        var offlineUser = Object.assign(_this3.room.onlineUsers.users[offlineUserIndex]);
        _this3.room.onlineUsers.users.splice(offlineUserIndex, 1);
        //添加离开消息
        var _message = new Message(offlineUser.id, offlineUser.nickname, " 离开房间", _this3.room.MessageType.CHAT);
        _this3.room.messages.push(_message);
      }
    }
    _this3.whenOnlineUserChange(_this3.room.onlineUsers);
  });
};

//查询和初始化在线用户列表和在线用户数
ChatRoomService.prototype.initialOnlineUsers = function (roomId) {var _this4 = this;
  var self = this;
  ...
  then(function (result) {
    if (result.code == 200) {
      ...
      result.content && result.content.map(function (onlineUser) {
        var userData = JSON.parse(onlineUser.userData);
        var user = new User(onlineUser.userId, userData.nickname, userData.avatar, "", 0, 0, 0);
        users.push(user);
      });
  ...
  then(function (result) {
    _this4.room.onlineUsers.count = result.content.onlineCount;
  }).catch(function (e) {
    console.log(e);
  });
};
...
ChatRoomService.prototype.sendMessages = function (roomId, content) {
  ...
  var message = this.im.createTextMessage(contentMessage);
...
/* 31 */
/*!************************************************************************************************!*\
  !*** C:/Users/pro/Desktop/GoEasyDemo-Uniapp-LiveChatRoom/js_sdk/u-charts/u-charts/u-charts.js ***!
  \************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
/* 39 */
/*!*****************************************************************************************!*\
  !*** C:/Users/pro/Desktop/GoEasyDemo-Uniapp-LiveChatRoom/components/uni-popup/popup.js ***!
  \*****************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
...
/* 40 */
/*!*******************************************************************************************!*\
  !*** C:/Users/pro/Desktop/GoEasyDemo-Uniapp-LiveChatRoom/components/uni-popup/message.js ***!
  \*******************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {
